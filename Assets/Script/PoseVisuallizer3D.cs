...
public class PoseVisuallizer3D : MonoBehaviour
{
    ...
    void OnRenderObject(){
        ...
        /*var puta = mainCamera.worldToCameraMatrix.inverse;// model.gameObject.transform.worldToLocalMatrix;
        puta[0, 3] = mirror.transform.position.x;
        puta[1, 3] = mirror.transform.position.y;
        puta[2, 3] = mirror.transform.position.z;*/
        material.SetMatrix("_invViewMatrix", mainCamera.worldToCameraMatrix.inverse) ;
        //material.SetMatrix("unity_MatrixVP", puta);
        //material.SetVector("_ScreenParams", model.transform.position);
